...
const SearchSelect = React.memo<TSearchSelect>(props => {
    ...
    */
    ...
    const loadOptions = (inputValue: string, callback: (options: OptionsOrGroups<TSelectOption, GroupBase<TSelectOption>>) => void) => {
        if (timeoutId) {
...
const ComponentSelect: React.FC<TComponentSelect> = (props) => {
    ...
    const handleChange = useCallback((value: TSelectValue) => {
        if (customHandler) {
            return customHandler(value)
        } else {
            let resolvedValue
    ...
    const handleAppendDuplicate = useCallback((currentLabel: TSelectOption, prevValue: Array<TSelectOption>) => {
