...
type TResolvedVariable = {title: string, variable: string}

type TResolvedTableVariables = Array<{title: string, name?: string, variables: Array<TResolvedVariable>}>

type TDefaultVariable = { title: string, field_type: string }

type TExtendedVariable = TDefaultVariable & { inner_variables?: { [key: string]: TDefaultVariable } }

type TVariablesObject = {
    title: string,
    variables: {
...
const getTableVariables = ({object, props, extendedTitle}: {object: string, props: TVariablesObject, extendedTitle: boolean}) => {
    const {title, variables} = props
    const variablesAsArray = Object.entries(variables)
    const resolvedVariables = variablesAsArray.filter(([key, variableProps]) => variableProps.field_type === "list" && variableProps.inner_variables)
    .map(([key, variableProps]) => ({
        title: extendedTitle ? `${title}. ${variableProps.title}` : variableProps.title,
        name: `${object}/${key}`,
        variables: Object.entries(variableProps.inner_variables as { [key: string]: TDefaultVariable }).map(([variableKey, innerVariableProps]) => ({
            title: innerVariableProps.title,
            variable: `${object}/${key}:${variableProps.field_type}:${variableKey}:${innerVariableProps.field_type}`
        }))
    }))
...
const ModuleDocuments: React.FC<ModuleDocumentsType> = (props) => {
    ...
    const variables = useMemo(() => {
        ...
        if (serverVariables) {
            const serverVariablesAsArray = Object.entries<TVariablesObject>(serverVariables)
            if (isFullVariablesList) {
                const resolvedVariables = serverVariablesAsArray.map(([sectionKey, sectionProps]) => ({
                    title: sectionProps.title,
                    name: sectionKey,
                    variables: Object.entries(sectionProps.variables).reduce<Array<TResolvedVariable>>((acc, currentValue) => {
                ...
                tableVariables = serverVariablesAsArray.reduce<TResolvedTableVariables>((acc, currentObject) => {
                    const [objectKey, objectVariables] = currentObject
                    const resolvedVariables = getTableVariables({object: objectKey, props: objectVariables, extendedTitle: true})

                    return acc.concat(resolvedVariables)
                }, [])
            } else {
                ...
                if (bindedVariables && bindedVariables.variables) {
                    const restructuredVariables = Object.entries(bindedVariables.variables)
                    ...
                    tableVariables = getTableVariables({object: variablesObject, props: bindedVariables, extendedTitle: false})
                }
            }
