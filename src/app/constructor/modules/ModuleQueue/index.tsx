...
import { TModuleQueueTalon, TModuleQueue, TFilters } from "./_types"
...
const Filters: React.FC<TFilters> = ({ filters, initialValues, handleChange }) => {
...
const ModuleQueueTalon: React.FC<TModuleQueueTalon> = (props) => {
...
const ModuleQueue: React.FC<TModuleQueue> = (props) => {
    ...
    */
    const [speakQueue, setSpeakQueue] = useState<Array<TModuleQueueTalon>>([])
    const [talonData, setTalonData] = useState<TModuleQueueTalon | null>(null)
    ...
    */
    const speak = useCallback(async (talon: TModuleQueueTalon) => {
        ...
        sound.addEventListener("loadedmetadata", () => {
            ...
            setTalonData(talon)
            if (!talon.is_alert) {
                api(talon.object, "update", { id: talon.id, is_alert: true })
            }
            setTimeout(() => {
