...
import { TApiNotificationTypes, TApiRole } from "../../../types/api"
import {
    TModuleRolesAdministrationAccess, TModuleRolesCard, TModuleRolesDetailsModal,
    TModuleRolesNotificationCheckbox, TModuleRolesNotificationTable, TModuleRolesPermissionCheckbox, TModuleRolesPermissionRow,
    TModuleRolesPermissionTable, TModuleRoles
} from "./_types"
...
const PermissionCheckbox: React.FC<TModuleRolesPermissionCheckbox> = ({ article, id, title, description }) => {
...
const AdministrationAccessCheckbox: React.FC<TModuleRolesAdministrationAccess> = ({ permissions_groups }) => {
    const intl = useIntl()
    const { values, setFieldValue } = useFormikContext<any>()
...
const PermissionRow: React.FC<TModuleRolesPermissionRow> = ({ group_title, permissions }) => {
    return <tr>
...
const PermissionsTable: React.FC<TModuleRolesPermissionTable> = ({ permissions }) => {
    const intl = useIntl()
...
const NotificationCheckbox: React.FC<TModuleRolesNotificationCheckbox> = ({ article, id, title, description }) => {
    const { values, setFieldValue } = useFormikContext<any>()
...
const NotificationsTable: React.FC<TModuleRolesNotificationTable> = ({ notifications }) => {
    const intl = useIntl()
    return <div className="fv-row">
...
const RoleDetailsModal: React.FC<TModuleRolesDetailsModal> = ({ chosenRole, setRole, refetchRoles }) => {
    const intl = useIntl()
    const { data: permissions } = useItem("permissions", {})

    const { data: notifications } = useItem<TApiNotificationTypes>("notificationTypes", {})
...
const RoleCard: React.FC<TModuleRolesCard> = ({ title, permissions, notificationTypes, article, setRole, handleRemoveRoleClick, id }) => {
    const intl = useIntl()
...
const ModuleRoles: React.FC<TModuleRoles> = () => {
    const { data, isLoading, refetch } = useItem<Array<TApiRole>>("roles", {})
    const [chosenRole, setRole] = useState<TApiRole | null>(null)
