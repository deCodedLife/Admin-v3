...
import {
    ModuleAppEditorComponentAreaType, ModuleAppEditorComponentBlockType, ModuleAppEditorComponentFieldType,
    ModuleAppEditorDraggableItemType, ModuleAppEditorFieldDataType, ModuleAppEditorFieldModalType,
    ModuleAppEditorFieldType, ModuleAppEditorHandleDropItemType, ModuleAppEditorSchemeModalType,
    ModuleAppEditorSchemeType, ModuleAppEditorType
} from "../../../types/modules"
...
const Field: React.FC<ModuleAppEditorComponentFieldType> = ({ field, draggableItem, handleEdit, onDrag, onDrop }) => {
    const intl = useIntl()
...
const Block: React.FC<ModuleAppEditorComponentBlockType> = ({ block, blockIndex, areaIndex, draggableItem, setDraggableItem, setFieldData, onDrop, handleDeleteBlock }) => {
    const intl = useIntl()
...
const Area: React.FC<ModuleAppEditorComponentAreaType> = (
    { area, areaIndex, selectedScheme, draggableItem, setDraggableItem, setFieldData, onDrop, handleAppendBlock, handleDeleteBlock, handleDeleteArea }) => {
    const intl = useIntl()
    const handleDragEnter = (event: React.DragEvent<HTMLDivElement>) => {
        switch (draggableItem?.type) {
            case "block":
...
const FieldModal: React.FC<ModuleAppEditorFieldModalType> = ({ fieldData, setFieldData, handleSubmitField, handleDeleteField }) => {
    const intl = useIntl()
...
const SchemeModal: React.FC<ModuleAppEditorSchemeModalType> = ({ showSchemeModal, setShowSchemeModal, handleAppendScheme }) => {
    const intl = useIntl()
    const validationSchema = Yup.object().shape({
        title: Yup.string().required(" "),
        article: Yup.string().matches(/^[A-Za-z_]+$/, intl.formatMessage({ id: "APP_EDITOR.ARTICLE_VALIDATION_SYMBOLS" })).min(4, intl.formatMessage({ id: "APP_EDITOR.ARTICLE_VALIDATION_LENGTH" })).required(" "),
...
const Scheme: React.FC<ModuleAppEditorSchemeType> = ({ scheme, selectedScheme }) => {
    const intl = useIntl()
    const [draggableItem, setDraggableItem] = useState<ModuleAppEditorDraggableItemType>(null)
    const { values, setFieldValue } = useFormikContext<any>()
    const [fieldData, setFieldData] = useState<ModuleAppEditorFieldDataType>(null)
    ...
    const handleSubmitField = (field: ModuleAppEditorFieldType) => {
        if (fieldData) {
    ...
    const handleDrop = (item: ModuleAppEditorHandleDropItemType) => {
...
const ModuleAppEditor: React.FC<ModuleAppEditorType> = ({ }) => {
    const intl = useIntl()
