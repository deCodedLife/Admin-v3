...
import { ModuleFunnelColumnType, ModuleFunnelItemType, ModuleFunnelType } from "../../types/modules"
...
const ModuleFunnelItem = React.memo<ModuleFunnelItemType>(({ id, title, description, tags, currentColumnId, handleOnDrag, handleClick }) => {
    const intl = useIntl()
...
const ModuleFunnelColumn: React.FC<ModuleFunnelColumnType> = ({ id, title, items, color, isItemsFetching, handleOnDrag, handleOnDrop, handleClick }) => {
...
const ModuleFunnel: React.FC<ModuleFunnelType> = (props) => {
    const intl = useIntl()
